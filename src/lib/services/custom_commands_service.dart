...
class CustomCommandsService {
  ...
  static Future<void> importCustomCommands(String jsonString, {bool replaceExisting = false}) async {
    try {
      ...
      final Set<String> existingcommandsSet = existingCommands.map((c) => c.command).toSet();
      
      for (final command in importedCommands) {
        if (!existingNames.contains(command.name) && !existingcommandsSet.contains(command.command)) {
