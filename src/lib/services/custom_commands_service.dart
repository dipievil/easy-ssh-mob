...
class CustomCommandsService {
  ...
  static Future<void> updateCustomCommand(
    CommandItem oldCommand,
    CommandItem newCommand,
  ) async {
    try {
      final commands = await loadCustomCommands();
      final index = commands.indexWhere(
        (c) => c.name == oldCommand.name && c.command == oldCommand.command,
      );
  ...
  static Future<void> importCustomCommands(
    String jsonString, {
    ...
    try {
      ...
      for (final command in importedCommands) {
        if (!existingNames.contains(command.name) &&
            !existingcommandsSet.contains(command.command)) {
